...
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('underscore-99xp'), require('backbone'), require('backbone-99xp'), require('validate-99xp'), require('app-exception')) :
    typeof define === 'function' && define.amd ? define(['exports', 'underscore-99xp', 'backbone', 'backbone-99xp', 'validate-99xp', 'app-exception'], factory) :
    (global = global || self, factory(global.BackboneORM = {}, global._, global.Backbone, global.bbx, global.v, global.AppException));
}(this, function (exports, _, Backbone, bbx, v, AppException) { 'use strict';
      ...
      defineEntity(definition) {
        ...
        switch (true) {
          case _.isArray(definition):
            o = conn.isDefined(definition[0]) ? conn.model(definition[0]) : conn.define(definition[0], definition[1] || {}, definition[2] || {});
            break;
    ...
    var extendedModel = {
      ...
      syncCreate(method, model, o) {
        ...
        var attrs = this.unformat(_.omit(_.clone(this.attributes), this.idAttribute)); // run insert

        return this.entity.create(attrs).then(r => success(r)).catch(_.bind(this.handleSyncError, this));
      },
      ...
      syncUpdate(method, model, o) {
        ...
        var attrs = this.unformat(_.omit(_.clone(this.attributes), this.idAttribute)); // build where

        var _o = {
          where: _.pick(this.attributes, this.idAttribute)
        }; // run update

        return this.entity.update(attrs, _o).then(r => success(r)).catch(_.bind(this.handleSyncError, this));
      },
